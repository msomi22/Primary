 ...
 *
 */
public class UpdateGradingScale extends HttpServlet{
   ...
   protected void doPost(HttpServletRequest request, HttpServletResponse response)
	   ...
	   if(StringUtils.isEmpty(A) || !isNumeric(A) || Integer.parseInt(A)>100){
    	   session.setAttribute(SessionConstants.SCHOOL_ERROR, ERROR_EMPTY_FIRLD); 
    	   ...
    	   GradingSystem gradingSystem = gradingSystemDAO.getGradingSystem(schooluuid);
    	   gradingSystem.setAccountUuid(schooluuid); 
    	   ...
    	   if(gradingSystemDAO.updateGradingSystem(gradingSystem)){ 
    		   session.setAttribute(SessionConstants.SCHOOL_SUCCESS, ERROR_SCALE_ADD_SUCCESS); 
    	   }else{
    		   session.setAttribute(SessionConstants.SCHOOL_ERROR, ERROR_SCALE_ADD_ERROR); 
    	   }
